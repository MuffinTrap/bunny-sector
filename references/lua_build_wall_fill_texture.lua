...
function Wonky_intersect(A1,A2,B1,B2)
...
function IntersectB(A1,A2,B1,B2)
	...
	local x=V2CrossProduct(
										V2New(AxA,toA1.x),
										V2New(BxB,toB1.x)
									)/det;
	local y=V2CrossProduct(
										V2New(AxA,toA1.y),
										V2New(BxB,toB1.y)
									)/det;
	return V2New(x,y)
...
FOVRAD=60*DEG2RAD -- VERTICAL FOV
HFOVRAD=72*DEG2RAD -- HORIZONTAL FOV
...
function CameraToScreen(x,y,z)
	...
	local ty=(y*NEARZ/z)*SH/VIEWPORT.y
	...
	if tA.x<0 and tB.x<0 then
		...
		if draw then
		 ...
		 print(string.format("BinCanv (%.2f,%.2f",Bct.x,Bct.y),8,nexty(),5)
			...
			if PRINT then
				print(string.format("Wall from %d->%d",math.floor(Act.x),math.floor(Bct.x)),8,nexty(),5)
			end

		if FILL then
				FillWall(Act.x,Bct.x,Act.y,Acb.y,
				Bct.y,Bcb.y,az,bz,1+(i%15))
			end
...
function FillWall(ax,bx,ayt,ayb,byt,byb,az,bz,col)

	-- Interpolate
	-- Make sure only draw inside screen
	local x1=math.floor(math.max(ax,-SW/2))
	local x2=math.floor(math.min(bx,SW/2-1))
	local yt1=ayt
	local yt2=byt
	local yb1=ayb
	local yb2=byb
	...
	local xchange=(bx-ax)
	-- How much y changes divided
	-- by how mych x changes
	-- gives y change for each x
	local ytchange=(yt2-yt1)/xchange
	local ybchange=(yb2-yb1)/xchange
	local zchange=(z2-z1)/xchange

	-- If wall starts before
	-- being visible on screen
	-- we need to do those before
	-- starting to draw
	local yt=yt1
	local yb=yb1
	local z=z1
	...
	if ax<x1 then
		local xoutside=x1-ax
		yt=yt+xoutside*ytchange
		yb=yb+xoutside*ybchange
		z=z+xoutside*zchange
	end
	local zleft=z
	local ytleft=yt
	local ybleft=yb
...
--	fovtest()
	...
	for x=-SW/2,SW/2-1 do
		line(SC.x+x,0,
		SC.x+x,(ZBUFFER[x]/FARZ)*SH/2,13)
	end
